...
void ba::BasicEffect::SetWorld(const XMMATRIX& matrix)
{
	world_->SetMatrix(reinterpret_cast<const float*>(&matrix));
}

void ba::BasicEffect::SetWorldInvTrans(const XMMATRIX& matrix)
{
	world_inv_trans_->SetMatrix(reinterpret_cast<const float*>(&matrix));
}

void ba::BasicEffect::SetTexMapping(const XMMATRIX& matrix)
{
...
void ba::BasicEffect::SetBoneTransforms(const XMFLOAT4X4* matrices, UINT count)
{
	bone_transforms_->SetMatrixArray(reinterpret_cast<const float*>(matrices), 0, count);
}

void ba::BasicEffect::SetEyePos(const XMFLOAT3& pos_w)
{
	eye_pos_->SetRawValue(&pos_w, 0, sizeof(XMFLOAT3));
}

void ba::BasicEffect::SetView(const XMMATRIX& matrix)
{
...
void ba::BasicEffect::SetFogRange(float range)
{
	fog_range_->SetFloat(range);
}

void ba::BasicEffect::SetFogColor(const XMVECTOR& color)
{
	fog_color_->SetFloatVector(reinterpret_cast<const float*>(&color));
}

void ba::BasicEffect::SetShadowMapSize(float size)
{
